    ...
    <div class="mx-auto px-6 max-w-lg w-full pt-10 pb-24 sm:pb-32 lg:flex lg:py-40 lg:px-8 items-center">
      <div class="w-full space-y-8">
        <div>
          ...
          <h2 class="mt-8 text-center text-3xl font-bold tracking-tight text-gray-50">Add more stress to your life!</h2>
          <p class="mt-2 text-center text-sm text-gray-300">
            or
            {{ ' ' }}
            <router-link to="login" class="font-medium text-indigo-500 hover:text-indigo-400">Changed your mind? Login's waiting.</router-link>
          </p>
        </div>
        <form @submit.prevent="onSubmit()" class="mt-8 space-y-6">
          ...
          <div class="-space-y-px rounded-md shadow-sm">
            <div>
              ...
              <input v-model="name" id="full-name" name="name" type="text" autocomplete="name" required class="relative block w-full appearance-none rounded-none rounded-t-md border border-gray-700 px-3 py-2 bg-gray-800 text-gray-50 placeholder-gray-600 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="Full name" />
            </div>
            <div>
              <label for="email-address" class="sr-only">Email address</label>
              <input v-model="email" id="email-address" name="email" type="email" autocomplete="email" required class="relative block w-full appearance-none rounded-none border border-gray-700 px-3 py-2 bg-gray-800 text-gray-50 placeholder-gray-600 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="Email address" />
            </div>
            <div>
